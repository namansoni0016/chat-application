...
const ChatSidebar = () => {
    ...
    return (
        <aside className="h-full w-20 lg:w-90 border-r border-base-300 flex flex-col transition-all duration-200">
            <div className="border-b border-base-300 bg-muted/40 w-full p-4">
            ...
            <div className="overflow-y-auto w-full">
                {users.map((user) => (
                    <button key={user.id} onClick={() => setSelectedUser(user.id)} className={`w-full p-1 flex items-center gap-2 transition-colors rounded ${selectedUser === user.id ? "bg-muted" : "hover:bg-base-300"}`}>
                        <div className="relative flex-shrink-0 mx-auto lg:mx-0">
                            <img src="/avatar.png" alt={user.name} className="size-11 object-cover rounded-full" />
                        </div>
                        {/* User info */}
                        <div className="hidden lg:flex flex-col  w-full">
                            <div className="font-bold text-start truncate">{user.name}</div>
